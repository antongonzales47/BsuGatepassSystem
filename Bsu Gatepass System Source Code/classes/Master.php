...
Class Master extends DBConnection {
	...
	function save_employee(){
		...
		$data = "";
		foreach($_POST as $k =>$v){
			if(in_array($k,array('employee_code','department_id','designation_id','fullname','status','vehicleNum'))){
				if(!is_numeric($v))
				$v= $this->conn->real_escape_string($v);
				if(!empty($data)) $data .=", ";
				$data .=" `{$k}` = '{$v}' ";
		...
		if($save){
			...
			$data = "";
			foreach($_POST as $k =>$v){
				if(in_array($k,array('id','employee_code','department_id','designation_id','fullname','status', 'vehicleNum')))
				continue;
				if(!empty($data)) $data .=", ";
			...
			if(!empty($data)){
				...
				if(!$save){
					...
					$resp['msg'] = 'Saving GatePass Details has failed. Error: '.$this->conn->error;
					$resp['sql'] = 	$sql2;
				}
			}
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," New GatePass was Successfully added.");
			}else{
				$this->settings->set_flashdata('success'," GatePass's Details Successfully updated.");
			}
		}
